...
Transition1 = namedtuple('Transition', ('speed', 'density'))
...
class ReplayMemory(Configurable):
    ...
    """
    def __init__(self, config=None, transition_type=Transition, transition_type2 = Transition1):
        super(ReplayMemory, self).__init__(config)
        self.capacity = int(self.config['memory_capacity'])
        self.transition_type = transition_type
        self.transition_type2 = transition_type2
        self.memory = []
        # self.memory_other = []
        self.position = 0
        # self.position_other = 0
    ...
    #         self.memory = self.memory[:self.capacity]
    #     # Faster than append and pop
    #     self.memory[self.position] = self.transition_type2(*args)
    #     self.position = (self.position + 1) % self.capacity

    # def push_other(self, *args):
    #     """Saves a transition."""
    #     if len(self.memory_other) < self.capacity:
    #         self.memory_other.append(None)
    #         self.position_other = len(self.memory_other) - 1
    #     elif len(self.memory_other) > self.capacity:
    #         self.memory_other = self.memory_other[:self.capacity]
    #     # Faster than append and pop
    #     self.memory_other[self.position_other] = self.transition_type(*args)
    #     self.position_other = (self.position_other + 1) % self.capacity
