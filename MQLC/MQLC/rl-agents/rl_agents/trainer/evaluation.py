...
class Evaluation(object):
    ...
    def step(self):
        ...
        # print(action)
        # start = time.time()
        self.observation, reward, terminal, info, rewards = self.monitor.step(action)
        # end = time.time()
        # print("obs:", end - start)

        # Record the experience.
        try:
            self.agent.record(previous_observation, action, rewards, self.observation, terminal, info)
