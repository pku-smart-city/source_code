...
import torch
import highway_env.envs.common.network as net
import os
import highway_env.envs.common.loader as dl
import highway_env.envs.common.util as ut
...
pre_model = torch.load("F:\python\workspace\MQLC\highway_env\highway_env\envs\model\IDM_model.pt")
pre_model.eval()
...
class KinematicObservation(ObservationType):
    ...
    FEATURES: List[str] = ["presence", "x", "y", "vx", "vy", "pre_x", "pre_y"]
    # FEATURES: List[str] = ["presence", "x", "y", "vx", "vy"]
    INTENTION_FEATURES: List[str] = ["x", "y"]
    # INTENTION_FEATURES: List[str] = ["presence", "x", "y", "vx", "vy", "cos_h", "sin_h", "type"]
    ...
    def __init__(self, env: 'AbstractEnv',
        ...
        self.intention_pre = intention_pre
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.features_range:
            ...
            self.features_range = {
                "x": [-50 * MDPVehicle.SPEED_MAX, 50 * MDPVehicle.SPEED_MAX],
    ...
    def pre_intention(self, obsfornn: np.ndarray) -> np.ndarray:
        selected_values = obsfornn[[2, 5, 8]][:,:,:]
        selected_values = selected_values.reshape(1, selected_values.shape[0], selected_values.shape[1],
                                                  selected_values.shape[2])
        with torch.no_grad():
            out = pre_model(selected_values)
            # print(out)
            out = out[0].numpy()
    ...
    def observe(self, obsfornn_all: list) -> np.ndarray:
        obsfornn_all_ndarray = np.empty((9, 5, 2))
        if not self.env.road:
        ...
        if 'pre_x' and 'pre_y' in self.features:
            ...
            obsfornn_all.append(df[self.intention_feature])

            if len(obsfornn_all) > 10:
                # b = time.time()
                last_elements = obsfornn_all[-9:]
                # print(last_elements)
                obsfornn_all_ndarray = np.reshape(last_elements, obsfornn_all_ndarray.shape)
                # print(obsfornn_all_ndarray)
                # c = time.time()
                # print("2", c - b)
                obsfornn_final = obsfornn_all_ndarray[-9:]
...
class MultiAgentObservation(ObservationType):
    ...
    def observe(self, obsfornn_all: list) -> tuple:
        return tuple(obs_type.observe(obsfornn_all) for obs_type in self.agents_observation_types)
