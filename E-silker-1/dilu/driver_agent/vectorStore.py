...
class DrivingMemory:
    ...
    def __init__(self, encode_type='sce_language', db_path=None) -> None:
        ...
        if encode_type == 'sce_encode':
            ...
            if os.environ["OPENAI_API_TYPE"] == 'azure':
                ...
                self.embedding = OpenAIEmbeddings()
            elif os.environ["OPENAI_API_TYPE"] == 'deepseek-R1-32B':
    ...
    ##############新增###############
    def retriveMemory_V1(self, driving_scenario: EnvScenario, frame_id: int,controlled_vehicle_id: int, top_k: int = 5):
        if self.encode_type == 'sce_encode':
            pass
        elif self.encode_type == 'sce_language':
            query_scenario = driving_scenario.describe_V1(controlled_vehicle_id, frame_id)
            similarity_results = self.scenario_memory.similarity_search_with_score(
                query_scenario, k=top_k)
            fewshot_results = []
            for idx in range(0, len(similarity_results)):
