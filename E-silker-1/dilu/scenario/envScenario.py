...
class EnvScenario:
    ...
    ##############新增###############
    def availableActionsDescription_V1(self, controlled_vehicle_id) -> str:
        avaliableActionDescription = 'Your available actions are: \n'


        # availableActions = self.env.llm_action_type.get_available_actions()
        # action_id=[]
        # for action in availableActions:
        #     action_id.append(action[controlled_vehicle_id])
        # action_id = list(set(action_id))

        action_id = self.env.llm_action_type.agents_action_types[controlled_vehicle_id].get_available_actions()
    ...
    def describe(self, decisionFrame: int) -> str:
        surroundVehicles = self.getSurrendVehicles(10)
        self.dbBridge.insertVehicle(decisionFrame, surroundVehicles)  #把当前帧的单车周围车辆信息写入数据库
    ...
    ##############新增###############
    def describe_V1(self, ego_number: int, decisionFrame: int) -> str:
        self.ego = self.env.llm_controlled_vehicles[ego_number]

        currentLaneIndex: LaneIndex = self.ego.lane_index
        roadCondition = self.processNormalLane(currentLaneIndex)    # 背景+车道信息
        SVDescription = self.describeSVNormalLane(currentLaneIndex)    #周围有效车辆以及自车的位置描述
