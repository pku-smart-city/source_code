...
class DiscreteMetaAction(ActionType):
    ...
    @property
    def vehicle_class(self) -> Callable:
        return functools.partial(MDPVehicle, target_speeds=self.target_speeds)  # 返回一个类，类的target_speeds参数固定
...
class MultiAgentAction(ActionType):
    def __init__(self, env: AbstractEnv, action_config: dict, **kwargs) -> None:
        ...
        for vehicle in self.env.llm_controlled_vehicles if self.action_config.get("controlled_count", None) else self.env.controlled_vehicles:
    ...
    def get_available_actions(self):
        return itertools.product( #做笛卡尔积
            *[
