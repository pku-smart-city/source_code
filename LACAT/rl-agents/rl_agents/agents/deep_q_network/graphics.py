...
class DQNGraphics(object):
    ...
    def display(cls, agent, surface, sim_surface=None, display_text=True):
        ...
        pygame.draw.rect(
            surface, cls.BLACK, (0, 0, surface.get_width(), surface.get_height()), 0
        )
        ...
        for action, value in enumerate(action_values):
            ...
            pygame.draw.rect(
                surface,
                color,
                (cell_size[0] * action, 0, cell_size[0], cell_size[1]),
                0,
            )
        ...
        try:
            ...
            for head in range(list(cls.v_attention.values())[0].shape[0]):
                ...
                for vehicle, attention in cls.v_attention.items():
                    ...
                    color = (
                        *color,
                        remap(attention[head], (0, 0.5), (100, 200), clip=True),
                    )
    ...
    def compute_vehicles_attention(cls, agent, state):
        ...
        attention = (
            agent.value_net.get_attention_matrix(state_t)
            .squeeze(0)
            .squeeze(1)
            .detach()
            .cpu()
        ...
        if hasattr(
            obs_type, "agents_observation_types"
        ):  # Handle multi-agent observation
...
class ValueFunctionViewer(object):
    ...
    def plot_value_map(self):
        ...
        values, actions = np.reshape(values, np.shape(xx)), np.reshape(
            actions, np.shape(xx)
        )
    ...
    def plot_values(self):
        ...
        self.values_history = (
            np.vstack((self.values_history, values))
            if self.values_history.size
            else values
        )
