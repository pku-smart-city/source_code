...
class ReplayMemory(Configurable):
    ...
    def __init__(
        self, config=None, transition_type=Transition, transition_type2=Transition1
    ):
        ...
        # self.memory_other = []
        self.position = 0
        self.idx_queue = deque(maxlen=100)
    ...
    def sample(self, batch_size, collapsed=True):
        if self.config["n_steps"] == 1:
            ...
            all_transitions = [
                self.memory[i : i + self.config["n_steps"]] for i in indexes
            ]
            # Collapse transitions
            return (
                map(self.collapse_n_steps, all_transitions)
                if collapsed
    ...
    def sample_joint(self, batch_size):
        indexes = random.sample(range(len(self.memory)), batch_size)
        all_transitions = [self.memory[i] for i in indexes]
