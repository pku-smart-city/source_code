...
class JointAtionValueNet(BaseModule, Configurable):
    def __init__(self, config):
        super().__init__()
        Configurable.__init__(self, config)
        self.net = nn.Sequential(
            nn.Linear(
                in_features=(self.config['in_total'] + self.config['vehicles']),
                out_features=self.config['layers'][0],
            ),
            nn.ReLU(),
            nn.Linear(in_features=self.config['layers'][0], out_features=1),
        )
        ...
        if self.config.get("out", None):
            if input_num == 4:
                self.predict = nn.Linear(
                    sizes[-1] + hidden_dim + hidden_dim_glo, self.config["out_total"]
                )
                # self.predict = nn.Linear(sizes[-1] + hidden_dim, self.config["out_total"])
            else:
                self.predict = nn.Linear(
                    sizes[-1] + hidden_dim + hidden_dim_glo, self.config["out"]
                )
    ...
    def forward(self, x):
        ...
        if len(points_list.shape) > 2:
            ...
            for v1 in v_list:
                ...
                input_data = torch.tensor([a, b], device='cuda:0', dtype=torch.float32)
            ...
            distances_1 = torch.norm(
                points_list[:, None, :, :] - points_list[:, :, None, :], dim=-1
            )
            ...
            a = average_speed.item().__float__()
            b = vehicle_density.item().__float__()
            input_data = torch.tensor([a, b], device='cpu', dtype=torch.float32)
            glo_en = self.global_layers(input_data)
            glo_en = self.relu(glo_en).unsqueeze(0)
        ...
        if self.config["reshape"]:
            x = x.reshape(
                x.shape[0], -1
            )  # We expect a batch of vectors 变成32×25或1×25的
        for layer in self.layers:
            x = self.activation(layer(x))
    ...
    def forward(self, points_list):
        ...
        if len(points_list.shape) > 2:
            ...
            for points in points_list:
                ...
                threshold = 0.3  # 距离阈值
                adj_matrix = (distances < threshold).astype(
                    int
                )  # 基于距离阈值构建邻接矩阵
...
class DuelingNetwork(BaseModule, Configurable):
    ...
    def forward(self, x):
        ...
        return (
            value
            + advantage
            - advantage.mean(1).unsqueeze(1).expand(-1, self.config["out"])
        )
...
class EgoAttention(BaseModule, Configurable):
    def __init__(self, config):
        ...
        self.value_all = nn.Linear(
            self.config["feature_size"], self.config["feature_size"], bias=False
        )
        self.key_all = nn.Linear(
            self.config["feature_size"], self.config["feature_size"], bias=False
        )
        self.query_ego = nn.Linear(
            self.config["feature_size"], self.config["feature_size"], bias=False
        )
    ...
    def forward(self, ego, others, mask=None):
        ...
        input_all = torch.cat(
            (ego.view(batch_size, 1, self.config["feature_size"]), others), dim=1
        )
        # Dimensions: Batch, entity, head, feature_per_head
        key_all = self.key_all(input_all).view(
            batch_size, n_entities, self.config["heads"], self.features_per_head
        )
        value_all = self.value_all(input_all).view(
            batch_size, n_entities, self.config["heads"], self.features_per_head
        )
        query_ego = self.query_ego(ego).view(
            batch_size, 1, self.config["heads"], self.features_per_head
        )
        ...
        result = (
            self.attention_combine(
                value.reshape((batch_size, self.config["feature_size"]))
            )
            + ego.squeeze(1)
        ) / 2
...
class SelfAttention(BaseModule, Configurable):
    ...
    def forward(self, ego, others, mask=None):
        ...
        result = (
            self.attention_combine(
                value.reshape((batch_size, n_entities, self.config["feature_size"]))
            )
            + input_all
        ) / 2
        return result, attention_matrix
...
class EgoAttentionNetwork(BaseModule, Configurable):
    def __init__(self, config):
        ...
        self.config["output_layer"]["in"] = self.config["attention_layer"][
            "feature_size"
        ]
        ...
        if self.config["self_attention_layer"]:
            self.self_attention_layer = SelfAttention(
                self.config["self_attention_layer"]
            )
        ...
        if mask is None:
            mask = (
                x[
                    :,
                    :,
                    self.config["presence_feature_idx"] : self.config[
...
class AttentionNetwork(BaseModule, Configurable):
    ...
    def forward(self, x):
        ego, others, mask = self.split_input(x)
        ego_embedded_att, _ = self.attention_layer(
            self.embedding(ego), self.others_embedding(others), mask
        )
        return self.output_layer(ego_embedded_att)
    ...
    def get_attention_matrix(self, x):
        ego, others, mask = self.split_input(x)
        _, attention_matrix = self.attention_layer(
            self.embedding(ego), self.others_embedding(others), mask
        )
        return attention_matrix
...
def size_model_config(env, model_config):
    ...
    """
    vehicles = len(env.controlled_vehicles)
    ...
    if model_config["type"] == "ConvolutionalNetwork":  # Assume CHW observation space
        ...
        model_config['vehicles'] = int(vehicles)
        model_config["in"] = int(np.prod(obs_shape))
        model_config["in_other"] = int((vehicles - 1) * np.prod(obs_shape))
        model_config["in_global"] = int(2)
        model_config["in_total"] = int(vehicles * np.prod(obs_shape))
    ...
    if isinstance(env.action_space, spaces.Discrete):
        ...
        model_config["out_total"] = int(5**vehicles)


def model_factory(config: dict, num_inputs: int = 1, joint=False) -> nn.Module:
    if config["type"] == "MultiLayerPerceptron":
        return MultiLayerPerceptron(config, num_inputs)
    if config["type"] == "MultiLayerPerceptron_he" and not joint:
        return MultiLayerPerceptron_he(config, num_inputs)
    elif config["joint_type"] == "JointAtionValueNet":
        return JointAtionValueNet(config)
