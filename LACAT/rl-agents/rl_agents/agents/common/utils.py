...
def least_used_device():
    ...
    if shutil.which('nvidia-smi') is None:
        raise RuntimeError(
            "nvidia-smi unavailable: cannot select device with most least memory used."
        )

    memory_map = get_gpu_memory_map()
    device_id = 1
    logger.info(
        "Choosing GPU device: {}, memory used: {}".format(
            device_id, memory_map[device_id]
        )
    )
...
def choose_device(preferred_device, default_device="cpu"):
    try:
        ...
        logger.warning(
            "Preferred device {} unavailable, switching to default {}".format(
