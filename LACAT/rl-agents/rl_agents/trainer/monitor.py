...
class MonitorV2(Monitor):
    ...
    def seed(self, seed=None):
        ...
        self.stats_recorder.seed = seeds[
            0
        ]  # Not sure why gym envs typically return *a list* of one seed
        return seeds
    ...
    def load_results(training_dir):
        ...
        for file in stats_files:
            with open(file) as f:
                content = json.load(f)
                content.update(
                    {'manifests': manifests, 'env_info': env_info, 'videos': videos}
                )
                if not all_contents:
...
class StatsRecorderV2(StatsRecorder):
    def __init__(
        self, directory, file_prefix, autoreset=False, env_id=None, log_infos=True
    ):
