...
class Evaluation(object):
    ...
    def run_episodes(self):
        for self.episode in range(self.num_episodes):
            ...
            while not terminal:
                reward, rewards, terminal = self.step()
                rewards.append(reward)
                ind_rewards.append(rewards)
    ...
    def step(self):
        mean_ttc = self._process_ttc()
        if mean_ttc >= self.args.ttc_parma and self.budget >= 0:
            actions = self.agent.plan(
                self.observation, [self.eps, self.exp_id, self.budget]
            )
            self.budget -= 1
        else:
            actions = self.agent.plan(self.observation, [self.eps, self.exp_id, -1])
        if not actions:
        ...
        self.observation, reward, terminal, info, rewards, self.ttc = self.monitor.step(
            action
        )
        # Record the experience.
        try:
            self.agent.record(
                previous_observation, action, rewards, self.observation, terminal, info
            )
            # self.agent.record_joint(previous_observation, action, rewards, self.observation, terminal, info)
        except NotImplementedError:
            pass

        return reward, rewards, terminal

    def _process_ttc(self):
        self.ttc[np.isinf(self.ttc)] = 1e18
        reciprocal_ttc = 1 / self.ttc
        mean_ttc = np.mean(reciprocal_ttc)
        return float(mean_ttc)
    ...
    def run_batched_episodes(self):
        ...
        batch_sizes = near_split(
            self.num_episodes * episode_duration,
        ...
        for batch, batch_size in enumerate(batch_sizes):
            ...
            logger.info(
                "Collecting {} samples with {} workers...".format(
            ...
            for trajectory in trajectories:
                if trajectory[
                    -1
                ].terminal:  # Check whether the episode was properly finished before logging
    ...
    @staticmethod
    def collect_samples(
        environment_config, agent_config, count, start_time, seed, model_path, batch
    ):
        """
    ...
    def save_agent_model(self, identifier, do_save=True):
        ...
        episode_path = None
        if do_save:
            episode_path = Path(self.monitor.directory) / "checkpoint-{}.tar".format(
                identifier
            )
            try:
    ...
    def after_all_episodes(self, episode, rewards, duration):
        ...
        logger.info(
            "Episode {} score: {:.1f} attack budget: {}".format(
                episode, sum(rewards), self.budget
            )
        )

    def after_all_episodes_other(self, episode, rewards, idx):
        rewards = np.array(rewards)
        gamma = self.agent.config.get("gamma", 1)
        self.writer.add_scalar(f'episode/length_{idx}', len(rewards), episode)
        self.writer.add_scalar(f'episode/total_reward_{idx}', sum(rewards), episode)
        self.writer.add_scalar(
            f'episode/return_{idx}',
            sum(r * gamma**t for t, r in enumerate(rewards)),
            episode,
        )
        self.writer.add_histogram(f'episode/rewards_{idx}', rewards, episode)

    def after_some_episodes(
        self, episode, rewards, best_increase=1.1, episodes_window=50
    ):
        ...
        if self.training:
            ...
            self.filtered_agent_stats += (
                1 / episodes_window * (np.sum(rewards) - self.filtered_agent_stats)
            )
            if (
                self.filtered_agent_stats > best_increase * best_reward
    ...
    def default_directory(self):
        return (
            ...
            / self.env.unwrapped.__class__.__name__
            / self.agent.__class__.__name__
        )

    @property
    def default_run_directory(self):
        return self.RUN_FOLDER.format(
    ...
    def reset(self):
        self.observation, self.ttc = self.monitor.reset()
        self.agent.reset()
