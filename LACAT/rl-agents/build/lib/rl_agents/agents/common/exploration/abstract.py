...
class DiscreteDistribution(Configurable, ABC):
    ...
    def sample(self):
        ...
        return self.np_random.choice(list(distribution.keys()), 1, p=np.array(list(distribution.values())))
    ...
    def sample_total(self):
        ...
        """
        distribution = self.get_distribution()
        return self.np_random.choice(list(distribution.keys()), 2, p=np.array(list(distribution.values())))
