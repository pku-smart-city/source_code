...
agents = {
    "olop": {
        ...
        "gamma": gamma,
        "upper_bound": {"type": "hoeffding", "c": 4},
...
def compare_agents(
    ...
    for agent, agent_name in evaluate_agents(env, agents, budget, seed):
        ...
        if isinstance(env, GridEnv):
            state_occupations[agent_name] = np.zeros(
                (2 * state_limits + 1, 2 * state_limits + 1)
            )
            state_updates[agent_name] = np.zeros(
                (2 * state_limits + 1, 2 * state_limits + 1)
            )
        ...
        if show_tree:
            TreePlot(agent.planner, max_depth=12).plot(
                out / "tree_{}.pdf".format(agent_name), title=rename(agent_name)
            )
            plt.show()
    ...
    if show_states:
        ...
        if v_max > 0:
            for agent_name, occupations in state_occupations.items():
                show_state_map(
                    "occupations", agent_name, occupations, state_limits, v_max
                )
    ...
    if show_trajs:
        ...
        for agent_name, agent_trajectories in trajectories.items():
            axes = show_trajectories(
                agent_name, agent_trajectories, axes=axes, color=next(palette)
            )
        plt.show()
