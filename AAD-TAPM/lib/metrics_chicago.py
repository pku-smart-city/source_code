...
def mask_mse_np(y_true, y_pred, region_mask, null_val=None):
    ...
    if null_val is not None:
        ...
        mask = region_mask
    mask = np.load('data/chicago/197_chicago.npy')
    mask /= mask.mean()
...
def Recall_use(y_true, y_pred, region_mask):
    """
    Arguments:
        y_true {np.ndarray} -- shape (samples,pre_len,W,H)
        y_pred {np.ndarray} -- shape (samples,pre_len,W,H)
    ...
    for i in range(len(true_top_k)):
        intersection = [v for v in true_top_k[i] if v in pred_top_k[i]]
        # print(accident_grids_nums[i])
        if accident_grids_nums[i] == 0:
            ko = 1
        else:
            ko = len(intersection) / accident_grids_nums[i]
        hit_all.append(ko)
        hit_sum += len(intersection)
    return hit_sum / sum(accident_grids_nums) * 100, np.array(hit_all).reshape(len(true_top_k), 1)
