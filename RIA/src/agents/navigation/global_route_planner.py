...
class GlobalRoutePlanner(object):
    ...
    def __init__(self, wmap, sampling_resolution=None):
        ...
        # - New API: GlobalRoutePlanner(world_map, sampling_resolution)
        # - Legacy API: GlobalRoutePlanner(GlobalRoutePlannerDAO)
        if sampling_resolution is None:
            if hasattr(wmap, "get_resolution"):
                sampling_resolution = wmap.get_resolution()
            else:
                raise TypeError("sampling_resolution is required when planner input has no get_resolution()")
    ...
    def setup(self):
        """
        Legacy no-op for callers that still invoke grp.setup().
        Graph construction is already done in __init__.
        """
        return
