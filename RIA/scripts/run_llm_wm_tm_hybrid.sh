...
# Hybrid runner:
# - Base behavior budget aligned with 2026-02-20 stable line
# - Keeps 2026-02-22 complex TM recovery stack
# - Re-enables LLM+WM decision loop in TM mode via periodic agent.run_step
...
LOG_ROOT="${LOG_ROOT:-src/experiments/wm_hybrid}"
SEED="${SEED:-42}"
TM_PORT="${TM_PORT:-8000}"

# Keep infra light by default; keyframes still saved per route end.
...
# Allow additional CLI flags override.
if [[ "$#" -gt 0 ]]; then
  CMD+=("$@")
fi

printf 'Preset: 2-20 base + 2-22 TM hybrid\n'
printf 'WM in loop: on\n'
printf 'WM subaction: on\n'
printf 'TM action hint: on\n'
