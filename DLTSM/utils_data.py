...
def PrepareDataset(speed_matrix, \
    ...
    if data_amount != 0:
        train_data = train_data[-data_amount * 24 * 12:, :, :, :]
        train_label = train_label[-data_amount * 24 * 12:, :, :]
    ...
    return train_dataloader, valid_dataloader, test_dataloader, max_speed, X_mean, train_data,train_label


def TrainModel(model, train_dataloader, valid_dataloader, A, learning_rate=1e-5, optm='Adam', num_epochs=500,
    ...
    for epoch in range(0, num_epochs):
        ...
        for data in train_dataloader:
            ...
            if len(labels[labels == 0]):
                ...
                loss_train = (((outputs * label_mask - torch.squeeze(labels)) ** 2) )
            else:
                loss_train = (((outputs - torch.squeeze(labels)) ** 2) )
...
def TestModel(model, test_dataloader, A,max_speed):
