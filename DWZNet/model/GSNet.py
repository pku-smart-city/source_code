...
class STGeoModule(nn.Module):
    def __init__(self,grid_in_channel,num_of_gru_layers,seq_len,
        ...
        # self.grid_gru = nn.GRU(input_size=grid_in_channel,hidden_size= int(gru_hidden_size/2),num_layers= num_of_gru_layers,batch_first=True,bidirectional =True) # v9
    ...
    def forward(self,grid_input,target_time_feature):
        ...
        grid_att_fc1_output = torch.squeeze(self.grid_att_fc1(gru_output)) # [25600, 6, 128] -> [25600, 6, 1] -> [25600, 6]
        grid_att_fc2_output = self.grid_att_fc2(grid_target_time) # [25600, 33] -> 
...
class STSemModule(nn.Module):
    def __init__(self,num_of_graph_feature,nums_of_graph_filters,
        ...
        # self.graph_gru = nn.GRU(input_size=num_of_filter,hidden_size=int(gru_hidden_size/2),num_layers= num_of_gru_layers,batch_first=True,bidirectional =True) # v9
    ...
    def forward(self,graph_feature,road_adj,risk_adj,poi_adj,
        ...
        graph_output = graph_output.permute(0,2,1).view(batch_size,-1,self.north_south_map,self.west_east_map)
        return graph_output
...
class GSNet(nn.Module):
    def __init__(self,grid_in_channel,num_of_gru_layers,seq_len,pre_len,
                ...
                north_south_map,west_east_map,node_dim):
        """[summary]
        ...
        self.gs_outdim = node_dim # gsnet -> sfmgtl 参数实验
        self.output_layer = nn.Linear(fusion_channel*north_south_map*west_east_map,self.gs_outdim*north_south_map*west_east_map)
        
        #sfmgtl
        hidden_dim = self.gs_outdim
        self.mlp = nn.Sequential(nn.Linear(self.gs_outdim+33, hidden_dim), #(127,32) 127 = hidden_dim*3+31
                            nn.LeakyReLU(),
                            nn.Linear(hidden_dim, hidden_dim))
        self.semantic1 = nn.Linear(self.gs_outdim, self.gs_outdim, bias=False)
        self.semantic2 = nn.Linear(self.gs_outdim, self.gs_outdim, bias=False)
        self.semantic3 = nn.Linear(self.gs_outdim, self.gs_outdim, bias=False)
        self.fuse_weight = nn.Parameter(torch.randn(1, self.gs_outdim)).unsqueeze(0)
    ...
    def forward(self,grid_input,target_time_feature,graph_feature,
        ...
        Arguments:
            ...
            poi_adj {np.array} -- poi adjacent matrix，shape：(N,N)
            grid_node_map {np.array} -- map graph data to grid data,shape (W*H,N)
            adj1,adj2,adj3: risk_adj, road_adj, poi_adj
        Returns:
            {Tensor} -- shape：(batch_size,pre_len,north_south_map,west_east_map)
        """
        ...
        grid_output = self.st_geo_module(grid_input,target_time_feature) # [32, 32, 20, 20]
        graph_output = self.st_sem_module(graph_feature,road_adj,risk_adj,poi_adj,
                                        target_time_feature) # [32, 32, 20, 20]
        
        grid_output = self.grid_weigth(grid_output) # [32, 16, 20, 20]
        graph_output = self.graph_weigth(graph_output)
        fusion_output = (grid_output + graph_output).view(batch_size,-1)
        fused_node = self.output_layer(fusion_output)\
                            .view(batch_size,self.north_south_map*self.west_east_map,self.gs_outdim) # bs,400,32 
        # fusion_output = fusion_output.view(batch_size,self.north_south_map*self.west_east_map,-1) # [32, 400, 16]
        isRec_adj = True
        if isRec_adj:
            # 在gsnet输出后面，添加t
            # fusion_output = fused_node
            # t = target_time_feature.unsqueeze(dim=1).repeat(1,self.north_south_map*self.west_east_map,1) 
            # concat_node = torch.cat([fusion_output, t], dim=-1)
            ...
            rec_adj3 = F.sigmoid(torch.bmm(self.semantic3(fused_node), fused_node.transpose(1, 2))) # [64, 400, 400]

            road_adj = torch.from_numpy(road_adj).float().to(rec_adj1.device)
            risk_adj = torch.from_numpy(risk_adj).float().to(rec_adj1.device)
            poi_adj = torch.from_numpy(poi_adj).float().to(rec_adj1.device)
            rec_loss = F.mse_loss(rec_adj1, road_adj) + F.mse_loss(rec_adj2, risk_adj) + F.mse_loss(rec_adj3, poi_adj) # road_adj.shape = (400,400)
            # rec_loss = 0
            fuse_weight = self.fuse_weight.to(fused_node.device)
            weighted_fuse_node = fused_node * fuse_weight # Zf*W
            fused_adj = F.softmax(F.relu(torch.bmm(weighted_fuse_node, weighted_fuse_node.transpose(1, 2))), dim=-1) # 公式（4）
